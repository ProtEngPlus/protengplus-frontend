...
export function ExportMutationResultOverlay({
  ...
  const handleExportMutationResult = async (isBookmarkOnly: boolean) => {
    try {
      ...
      link.href = url;
      link.download = `mutaiton_results_${
        isBookmarkOnly ? "bookmark_" : ""
      }${mutationId}.csv`;
  ...
  return (
    isVisible && (
      ...
      >
        <div
          className="modal-container max-w-[700px] w-fit bg-white opacity-100 border border-pep-gray-border rounded-lg px-10 py-8 space-y-11 m-auto shadow-dropShadow z-[100] text-center"
          id={`${id}`}
        >
