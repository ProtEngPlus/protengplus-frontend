...
import {
  ...
  RenameMutationProps,
} from "../Overlay/RenameMutationOverlay";
...
const headers = ["Collection Name", "Status"];
...
export default function MutationTable({
  ...
  const handleBookmark = async (mutation: MutationInterface) => {
    await updateMutationDetail(mutation.id, {
      is_bookmark: !mutation.is_bookmark,
    });
    refresh();
  };
          ...
          <div className="overflow-auto rounded-xl border border-[#DFE4EA] shadow-table w-full max-h-[500px] overflow-y-scroll">
            <table className="w-full text-xs text-left rtl:text-right">
              <thead className="leading-6 bg-[#F9FAFB] text-left border-b sticky top-0 h-[60px] z-20">
                <tr>
                  <th className="font-normal px-6 py-3 text-base"></th>
                  ...
                  >
                    ...
                    <td className="px-3 py-3">
                      <div className="flex flex-col items-center w-fit">
                        <MutationState
                          state={mutation.state as MutationStateType}
                          className="mx-auto"
                        />
                        {mutation.state === "COMPLETED" &&
                          mutation.complete_at && (
                            <label>
                              Total Time:{" "}
                              {getTotalTime(
                                mutation.created_at,
